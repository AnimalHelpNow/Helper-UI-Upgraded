...
/**
 * Build the display name (title) from the First+Middle+Last names or organization name.
 */
Drupal.behaviors.openpublic_person_name = {
  ...
  //Drupal.settings.openpublic_person.firstName
   ...
   var middlename = Drupal.settings.openpublic_person.middleName;
   ...
   var orgname = Drupal.settings.openpublic_person.orgName;
    ...
    var sources = {
      ...
      middlename: $(middlename, context).addClass('display-name-source'),
    ...
    for (key in sources) {
      sources[key].bind('keyup.displayName change.displayName', function () {
        ...
        content = (sources['firstname'].val() || sources['lastname'].val()) ? sources['firstname'].val() + ' ' + sources['middlename'].val() + ' ' + sources['lastname'].val() : sources['orgname'].val();
